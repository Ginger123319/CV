...
def calculate_map(work_dir):

    wd = os.getcwd()
    evaluate_result_path = work_dir + '/middle_dir_ssd_dis/evaluate_result'

    map_dict = {}
    # model_dir_list = os.listdir(dc.conf.inputs.model_dir)
    ...
    for model_index in model_dir_list:
        if model_index.endswith('.pth'):
            ...
            GT_PATH = os.path.join(os.getcwd(), work_dir + '/middle_dir_ssd_dis', 'image_info', 'ground_truth')
            DR_PATH = os.path.join(os.getcwd(), work_dir + '/middle_dir_ssd_dis', 'image_info',
                                   'detect_result_' + str(model_index))
            # if there are no images then no animation can be shown
            IMG_PATH = os.path.join(os.getcwd(), work_dir + '/middle_dir_ssd_dis', 'image_info',
                                    'images_optional_' + str(model_index))
            ...
            if not args.no_plot:
                try:
                    ...
                    print(r"matplotlib not found, please install it to get the resulting plots.")
                    args.no_plot = True
            ...
            results_files_path = evaluate_result_path + "/results_" + str(model_index)
            ...
            # get a list with the detect-result files
            ...
            for txt_file in ground_truth_files_list:
                replace_txt_file = txt_file.replace('ground_truth', 'detect_result_' + str(model_index))
                if replace_txt_file not in dr_files_list:
                    continue
                # print(txt_file)
                ...
                for line in lines_list:
                    try:
                        ...
                        if "difficult" in line:
                            if len(label_line) == 6:
                                class_name, left, top, right, bottom, _difficult = label_line
                            else:
                                (_difficult, bottom, right, top, left), class_name = label_line[::-1][0:5], " ".join(label_line[::-1][5:][::-1])
                            is_difficult = True
                        else:
                            if len(label_line) == 5:
                                class_name, left, top, right, bottom = label_line
                            else:
                                (bottom, right, top, left), class_name = label_line[::-1][0:4], " ".join(label_line[::-1][4:][::-1])
                    except ValueError:
                        error_msg = "Error: File " + txt_file + " in the wrong format.\n"
                        error_msg += " Expected: <class_name> <left> <top> <right> <bottom> ['difficult']\n"
                        error_msg += " Received: " + line
                        error_msg += "\n\nIf you have a <class_name> with spaces between words you should remove them\n"
                        error_msg += "by running the script \"remove_space.py\" or \"rename_class.py\" in the " \
                                     "\"extra/\" folder. "
                        error(error_msg)
            ...
            for class_index, class_name in enumerate(gt_classes):
                ...
                for txt_file in dr_files_list:
                    ...
                    for line in lines:
                        try:
                            tmp_label_line = line.split()
                            if len(tmp_label_line) == 6:
                                tmp_class_name, confidence, left, top, right, bottom = tmp_label_line
                            else:
                                (bottom, right, top, left, confidence), tmp_class_name = tmp_label_line[::-1][0:5], " ".join(tmp_label_line[::-1][5:][::-1])
                        except ValueError:
                            error_msg = "Error: File " + txt_file + " in the wrong format.\n"
                    ...
                    for idx, detection in enumerate(dr_data):
                        ...
                        if show_animation:
                            ...
                            if ovmax > 0:  # if there is intersections between the bounding-boxes
                                ...
                                cv2.putText(img_cumulative, class_name, (bbgt[0], bbgt[1] - 5), font, 0.6, light_blue,
                                            1, cv2.LINE_AA)
                            bb = [int(i) for i in bb]
                            cv2.rectangle(img, (bb[0], bb[1]), (bb[2], bb[3]), color, 2)
                            cv2.rectangle(img_cumulative, (bb[0], bb[1]), (bb[2], bb[3]), color, 2)
                            cv2.putText(img_cumulative, class_name, (bb[0], bb[1] - 5), font, 0.6, color, 1,
                                        cv2.LINE_AA)
                            # show image
            ...
            with open(results_files_path + "/results.txt", 'a') as results_file:
                ...
                for class_name in sorted(dr_classes):
                    ...
                    class_tp_fp_dict['fp'] = str(n_det - count_true_positives[class_name])
            ...
            map_dict[model_index] = round(mAP, 3)

    print("map_dict:", map_dict)

    best_model = ""
    for model_index in map_dict.keys():
        if map_dict[model_index] == max(map_dict.values()):
