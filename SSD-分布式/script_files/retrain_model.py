...
def train(num_classes, image_size, val_size, lr, freeze_epoch, total_epoch, optimizer, batch_size, work_dir, tensorboard_dir):
    training = Train(num_classes, image_size, val_size, work_dir, tensorboard_dir )
    training.train(lr, freeze_epoch, total_epoch, optimizer, batch_size)

def select_best_model(work_dir):      
    if int(os.environ.get("RANK")) == 0:   
        # 筛选出val loss最低的一个pth文件
        logs_list_dir = os.listdir(work_dir+'/middle_dir_ssd_dis/logs')
        ...
        for i in val_key_list_1:
            shutil.copyfile(work_dir+'/middle_dir_ssd_dis/logs/'+i, work_dir+'/middle_dir_ssd_dis/normal_train_best_model_dir/'+i)
