...
# [{"left": 9, "top": 61, "right": 218, "bottom": 282, "label": "horse"}, {"left": 301, "top": 76, "right": 500, "bottom": 282, "label": "horse"}]
        ...
        try:
            ...
            for obj in root.iter('object'):
                ...
                if cls not in classes:
                    continue
                box = obj.find('bndbox')
                b = {'left': int(box.find('xmin').text), 'top': int(box.find('ymin').text), 'right': int(box.find('xmax').text),
            ...
            for anno in annos:
                ...
                if bbox and cat:
                    ...
                    if cat_str not in classes:
                        continue
                    x, y, w, h = bbox
                    target_data.append({"left": int(x), "top": int(y), "right": int(x + w), "bottom": int(y + h), "label": cat_str})
            if len(target_data) == 0:
