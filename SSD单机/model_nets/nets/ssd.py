...
from model_nets.nets.ssd_layers import Detect, L2Norm, PriorBox
from model_nets.nets.vgg import vgg as add_vgg
...
class SSD(nn.Module):
    ...
    def forward(self, x):
        with torch.no_grad():
            priors = Variable(self.priorbox.forward()).to(device=x.device).unsqueeze(0)
        sources = list()
        loc = list()
        conf = list()
