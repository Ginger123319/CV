...
from model_nets.utils.aug_util import get_random_data
...
class SSDDataset2(Dataset):
    def __init__(self, tf_record_file, index_file, image_size: tuple, aug_config=None, mosaic=False):
        super(SSDDataset2, self).__init__()
        self.mosaic = mosaic
        self.tf_record_file = tf_record_file
        self.index_file = index_file
        self.indexs = np.loadtxt(index_file, dtype=np.int64)
        self.sample_num = self.indexs.shape[0]
    ...
    def load_index_img(self, index):
        ...
        img_arr = np.frombuffer(feature['image'], dtype=np.uint8).reshape(img_shape)
        image = Image.fromarray(img_arr)
        y = np.frombuffer(feature['label'], dtype=float).reshape(y_shape).copy()
        if y.shape[0]==0:
            return None, None

        for i in range(5):
            img, y = get_random_data(image=image, box=y, input_shape=self.image_size[0:2], aug_config=self.aug_config, index=index)
            if y.shape[0]>0:
                break
        if y.shape[0]==0:
            return None, None
        ...
        #     img_draw.save(Path(draw_dir, "{}_ok.png".format(index)))        
