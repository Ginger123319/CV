...
# 绘制单张原图和增强后的图片及绘制目标框
def draw_rect(original_img, transform_img, boxes, option):
    for box in boxes:
        x1, y1, x2, y2 = box
...
def rotate_img_point(input_img, theta=45):
    ...
    # 标签框坐标转换
    boxes = transform_affine_labels(boxes, M, rotate_img)
    return rotate_img, boxes
...
def crop(input_img, x1=0.3, y1=0.3, x2=0.8, y2=0.8):
    ...
    if crop_x1 < crop_x2 <= w and crop_y1 < crop_y2 <= h:
        ...
        # 先做坐标转换
        boxes[:, [0, 2]] = boxes[:, [0, 2]] - crop_x1
        boxes[:, [1, 3]] = boxes[:, [1, 3]] - crop_y1
        boxes[:, [1, 3]] = np.clip(boxes[:, [1, 3]], 2, crop_y2 - crop_y1 - 2)
        boxes[:, [0, 2]] = np.clip(boxes[:, [0, 2]], 2, crop_x2 - crop_x1 - 2)
...
def scale_img(img, scale=0.5, is_Equal=True):
    ...
    # 修改标签
    boxes = transform_affine_labels(boxes, M, scale_img)
    return scale_img, boxes
...
def shearXY(image, shear=10):
    ...
    # 进行标签转换
    boxes = transform_affine_labels(boxes, M, affine_img)
    return affine_img, boxes
...
if __name__ == '__main__':
    ...
    if len(img[1]) > 0:
        ...
        draw_rect(image_list[:2][0][0], mix_img, boxes, "mix")
        # -----------------------------------------
        # 随机进行翻转
        # ------------------------------------------
        flip_img, boxes = flip(img)
        draw_rect(img[0], flip_img, boxes, "flip")
        # -----------------------------------------
        # 指定角度和中心点进行旋转
        # ------------------------------------------
        rotate_img, boxes = rotate_img_point(input_img=img)
        draw_rect(img[0], rotate_img, boxes, "rotate")

        # -----------------------------------------
        # 随机进行裁剪
        # ------------------------------------------
        crop_img, boxes = crop(img)
        draw_rect(img[0], crop_img, boxes, "crop")

        # -----------------------------------------
        # 对输入进来的图片进行缩放
        # ------------------------------------------
        scale = rand(scale_low, scale_high)
        scale_img, boxes = scale_img(img, scale=0.5, is_Equal=True)
        draw_rect(img[0], scale_img, boxes, "scale")
        exit()
        # -----------------------------------------
        # 对输入进来的图片进行随机色彩变换
        # ------------------------------------------
        ...
        draw_rect(img[0], color_img, boxes, "color_rgb")
        # -----------------------------------------
        # 对输入进来的图片进行随机的HSV变换
        # ------------------------------------------
        v_img, boxes = brighter(img)
        draw_rect(img[0], v_img, boxes, "brighter")
        s_img, boxes = saturation(img)
        draw_rect(img[0], s_img, boxes, "saturation")
        # hsv_img, boxes = augment_hsv(img)
        # draw_rect(img[0], hsv_img, boxes, "hsv")
        # -----------------------------------------
        # 对输入进来的图片随机添加椒盐噪声
        # np.copy深拷贝，图片存储在另一个地址
        # ------------------------------------------
        noise_img, boxes = random_noise(img)
        draw_rect(img[0], noise_img, boxes, "noise")
        # -----------------------------------------
        # 对输入进来的图片随机擦除
        # ------------------------------------------
        erase_img, boxes = random_erase(img)
        draw_rect(img[0], erase_img, boxes, "erase")
        # -----------------------------------------
        # 对输入进来的图片进行仿射变换
        # ------------------------------------------
        # affine_img, boxes = random_affine(img)
        # draw_rect(img[0], affine_img, boxes, "affine")
        # -----------------------------------------
        # 全局增强图片对比度
        # ------------------------------------------
        contrast_img, boxes = contrast(img)
        draw_rect(img[0], contrast_img, boxes, "contrast")
        # -----------------------------------------
        # 仿射变换-错切
        # ------------------------------------------
        shear_img, boxes = shearXY(img, shear=15)
        draw_rect(img[0], shear_img, boxes, "shear")
        # -----------------------------------------
        # 仿射变换-平移
        # ------------------------------------------
        translate_img, boxes = translateXY(img, translate=.2)
        draw_rect(img[0], translate_img, boxes, "translate")
