...
def calculate_performance(work_dir, class_list):
    f = open(work_dir + '/middle_dir/evaluate_result/results_' + '/results.txt')
    contents = f.read()
    f.close()
    ...
    score_dict = {}
    for i in class_list:
        each_calss_data = re.match(
            r'%s AP = (\d+.\d+).+score_threshold=(\d+.\d+) : F1=(.+) ; Recall=(\d+.\d+). ; Precision=('
            r'\d+.\d+).\sPrecision:(.+)\sRecall:(.+)\sF1:(.+)\sscore:(.+)\s\s' % ( 
                i), contents)
        if each_calss_data is None:
            print("Skip class: {}. Can't find score data.".format(i))
    ...
    detect_results_bar = {"type": "detect_results", "name": "detect_results",
                          "data": {"detect_results": detect_results_dict}}
