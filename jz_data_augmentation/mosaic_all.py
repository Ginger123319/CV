...
def merge_bboxes(bboxes, cutx, cuty):
    ...
    for i, box in enumerate(bboxes):
        ...
        for index, box in enumerate(box):
            ...
            # 更新坐标信息
            bboxes[i][index][:4] = [x1, y1, x2, y2]  # 更新第i张图的第index个检测框的坐标
...
# 对传入的四张图片数据增强
def get_mosaic_data(image_list, label_type, augment_scope=None):
    input_shape = (416, 416)
    ...
    for frame_list in image_list:
        ...
        if label_type == "103":
            if augment_scope == 'part':
                print("部分图片暂不支持马赛克！")
                return image_list[-1][0], image_list[-1][1]
            else:
    ...
    if label_type == "103":
        ...
        return new_image, box_datas.tolist()
    else:
        cls_list = list(set(cls_list))
        return new_image, cls_list
