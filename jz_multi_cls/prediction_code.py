...
class Predict:
    ...
    def predict(self):
        self.net.eval()
        preds = torch.zeros([len(self.test_data), len(self.test_data.class_name)])
        sample_id = torch.zeros(len(self.test_data))
        # 返回预测类别和sampleId
        with torch.no_grad():
            for i, (sampleId, images, tags, idx) in enumerate(self.test_loader):
                img_data = images.to(DEVICE)
                test_out = self.net.forward(img_data)
                outs = torch.sigmoid(test_out)
                outs = (outs > 0.5).float()
                sample_id[idx] = sampleId
                preds[idx] = outs.cpu()
        return preds, sample_id
