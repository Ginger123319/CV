...
class Train:
    ...
    def __call__(self, *args, **kwargs):
        ...
        for epoch in range(self.epochs):
            ...
            sum_loss = 0.
            for (_, images, tags, _), (_, unlabeled_images, _, _) in zip(self.train_loader, self.unlabeled_data_loader):
                self.net.train()
                # 计算有标签损失
                ...
                unlabeled_outputs = self.net.forward(unlabeled_images)
                unlabeled_probs, _ = torch.max(torch.softmax(unlabeled_outputs, dim=1), dim=1)
                unlabeled_loss = -torch.mean(torch.log(unlabeled_probs))
            ...
            print(f"有标签和无数据训练轮次：{epoch + 1}==========平均损失：{train_avg_loss}")

            # print("==========为未标注数据生成伪标签=========")
            # # 在无标签数据上生成伪标签
            # self.net.eval()
            # with torch.no_grad():
            #     data = np.array(self.unlabeled_dataset.data)
            #     for (_, inputs, tags, index) in self.unlabeled_data_loader:
            #         inputs = inputs.to(DEVICE)
            #         outputs = self.net(inputs)
            #         _, predicted = torch.max(outputs.data, 1)
            #         predicted = predicted.cpu().numpy()
            #         data[index, 2] = predicted
            #
            #     self.unlabeled_dataset.data = data.tolist()

            # # 使用无标签数据和伪标签进行数据增强和训练
            # sum_loss = 0.
            # for (_, inputs, tags, _) in self.unlabeled_data_loader:
            #     self.net.train()
            #
            #     inputs = inputs.to(DEVICE)
            #     tags = tags.to(DEVICE)
            #
            #     outputs = self.net(inputs)
            #     loss = self.loss_func(outputs, tags)
            #
            #     self.opt.zero_grad()
            #     loss.backward()
            #     self.opt.step()
            ...
            #
            # train_avg_loss = sum_loss / len(self.train_loader)
            # print(f"无标签数据训练轮次：{epoch + 1}==========平均损失：{train_avg_loss}")
            #
            # # 使用有标签数据进行微调
            # sum_loss = 0.
            ...
            #
            # train_avg_loss = sum_loss / len(self.train_loader)
            # print(f"有标签数据微调训练轮次：{epoch + 1}==========平均损失：{train_avg_loss}")
