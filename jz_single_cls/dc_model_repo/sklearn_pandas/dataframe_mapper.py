...
class DataFrameMapper(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, features, default=False, sparse=False, df_out=False,
                 input_df=False, preserve_origin_dtypes=False):
        """
        Params:
        ...
        self.preserve_origin_dtypes = preserve_origin_dtypes
    ...
    def __setstate__(self, state):
        ...
        self.preserve_origin_dtypes = state.get("preserve_origin_dtypes", False)
    ...
    def get_names(self, columns, transformer, x, alias=None):
        ...
        # 2. 列名增多的情况（OneHot这类,使用其内部记录的列名,其中没有找到带get_feature_names方法的共性。）
        from sklearn.preprocessing import OneHotEncoder
        from sklearn.preprocessing import PolynomialFeatures
        if isinstance(transformer, OneHotEncoder) or isinstance(transformer, PolynomialFeatures):
            feature_names = transformer.get_feature_names(columns)
            # Fixed by zk. 2020.9.10. 当前sklearn为0.20版本.
            # transformer为OneHotEncoder时，get_feature_names返回np.ndarray,可通过tolist转为list
            # transformer为PolynomialFeatures时，get_feature_names返回的本身就是list，再执行tolist会报错
            return feature_names.tolist() if not isinstance(feature_names, list) else feature_names

        # 3. 列名减少的情况（特征选择这类,使用其内部记录的mask计算列名）
        import sklearn
        if sklearn.__version__.startswith("0.24.") or sklearn.__version__.startswith("0.23."):
            from sklearn.feature_selection import SelectorMixin
        else:
            from sklearn.feature_selection.base import SelectorMixin
        if isinstance(transformer, SelectorMixin):
            cols_mask = transformer.get_support()
            if len(cols_mask) == len(columns):
                support_cols = []
                for i, v in enumerate(cols_mask):
                    if v:
                        support_cols.append(columns[i])
                if len(support_cols) == num_cols:
                    return support_cols
    ...
    def transform(self, X):
        ...
        # Fix bug for model_selection 2021.5.28
        origin_dtypes = X.dtypes if self.preserve_origin_dtypes else None
        ...
        if self.df_out:
            ...
            for col, dtype in zip(self.transformed_names_, dtypes):
                if dtype != df_out.dtypes[col]:
                    df_out[col] = df_out[col].astype(dtype)

            # Fix bug for model_selection 2021.5.28
            origin_dtypes = X.dtypes if self.preserve_origin_dtypes else None
            if self.preserve_origin_dtypes:
                cur_dtypes = df_out.dtypes
                for k, v in cur_dtypes.items():
                    if k in origin_dtypes and origin_dtypes[k]!=v:
                        df_out[k] = df_out[k].astype(origin_dtypes[k])
