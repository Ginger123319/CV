...
from classifier_multi.training_code import CSVDataset, Train, WeakStrongDataset
...
class MyModel(Model):
    ...
    def train_model(self, df_train, df_val, work_dir, is_first_train, **options):
        ...
        train_ratios = net_config["train_ratios"]
        print(f'partition_dir is {partition_dir}')
        ...
        df_train = CSVDataset(df_train, class_name, input_shape, image_col, label_col, id_col, partition_dir,
                              is_train=True)
        df_unlabeled = WeakStrongDataset(df_unlabeled, image_col, label_col, id_col, partition_dir)
        train = Train(df_train, df_val, df_unlabeled, self.net, batch_size, optimizer_type, epochs, train_ratios,
                      class_num)
        self.net = train(partition_dir)
    ...
    def split_train_val(df_init, df_anno, is_first_train, val_size=0.2, random_seed=1):
        ...
        # 计算每个类别的样本比例
        class_counts = df_train['label'].value_counts(normalize=True)
        class_counts.sort_index(inplace=True)

        # 输出结果
        train_ratios = torch.from_numpy(1 - class_counts.values)

        print("Train count: {}\n"
              "Val   count: {}".format(df_train.shape[0], 0 if df_val is None else df_val.shape[0]))
        return df_train, df_val, train_ratios
