...
from main_utils import FocalLoss
...
class WeakStrongDataset(Dataset):
    def __init__(self, input_df, image_col, label_col, id_col, partition_dir):
    ...
    def __getitem__(self, index):
        ...
        img = Image.open(img_path).convert('RGB')
        tag = self.data[index][2]
        return sample_id, weak_transform(img), strong_transform(img), np.int64(tag), index


# 自定义训练
class Train:
    def __init__(self, train_dataset, val_dataset, df_unlabeled, model, batch_size, optimizer_type, epochs,
                 train_ratios, class_num):
        ...
        # 未标注数据进行弱增强和强增强
        self.unlabeled_loader = DataLoader(df_unlabeled, shuffle=True, batch_size=batch_size, num_workers=2)
        ...
        # 创建损失函数
        self.loss_func = FocalLoss(class_num=class_num, alpha=train_ratios)  # 内部带有Softmax函数对输出进行归一化处理

    def __call__(self, *args, **kwargs):
        partition_dir = args[0]
        # 早停
        ...
        for epoch in range(self.epochs):
            ...
            sum_loss = 0.
            for (_, images, tags, _), (_, weak_img, strong_img, _, _) in zip(self.train_loader, self.unlabeled_loader):
                self.net.train()
                # 计算有标签损失
                ...
                loss1 = self.loss_func(out, tag_data)
                # 计算未标注数据一致性损失
                weak_img = weak_img.to(DEVICE)
                strong_img = strong_img.to(DEVICE)

                weak_out = self.net.forward(weak_img)
                strong_out = self.net.forward(strong_img)

                loss2 = torch.nn.functional.mse_loss(weak_out, strong_out)
                # 计算总损失
            ...
            if self.val is not None:
                ...
                self.writer.add_scalars('Val', {'acc': test_avg_score, 'loss': test_avg_loss}, epoch)
                print()
        if self.val is not None:
