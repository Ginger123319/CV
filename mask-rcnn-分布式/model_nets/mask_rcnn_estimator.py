...
class MaskRCNNEstimator(UserDefinedEstimator):
    ...
    def fit(self, X, y, **kwargs):
        ...
        training = Train(self.num_classes, self.image_size, self.val_size, self.work_dir, self.tensorboard_dir, self.dist, self.use_tfrecord, self.use_amp, mosaic=self.mosaic)
        training.train(self.lr, self.freeze_epoch, self.total_epoch, self.optimizer, self.batch_size)
        
        if int(os.environ.get("RANK")) == 0:
            # 筛选出val loss最低的一个pth文件
            logs_list_dir = os.listdir(self.work_dir+'/middle_dir_mask_dis/logs')
            ...
            for i in val_key_list:        
                os.system('cp %s %s' % (self.work_dir+'/middle_dir_mask_dis/logs/'+i, self.pth_logs_dir))
            ...
            for i in val_key_list_1:  
                os.system('cp %s %s' % (self.work_dir+'/middle_dir_mask_dis/logs/'+i, self.work_dir+'/middle_dir_mask_dis/normal_train_best_model_dir'))
    ...
    def predict(self, X, **kwargs):
        ...
        from model_nets.frcnn import FRCNN

        frcnn = FRCNN(self.step_path)
        ...
        for i in range(len(X)):
            ...
            image_info_list = frcnn.detect_image(image)
    ...
    def persist_model(self, fs, step_path):
        ...
        fs.copy(self.work_dir+'/middle_dir_mask_dis/data_info/classes.txt', step_data_path)
        fs.copy(self.work_dir+'/middle_dir_mask_dis/logs/'+self.best_model_pth, step_data_path)
