...
class MaskRCNNEstimator(UserDefinedEstimator):
    ...
    def fit(self, X, y, **kwargs):
        ...
        training = Train(self.num_classes, self.image_size, self.val_size, self.work_dir, self.tensorboard_dir, self.use_tfrecord, self.use_amp, mosaic=self.mosaic)
        ...
        for i in val_key_list_1:  
            os.system('cp %s %s' % (self.work_dir+'/middle_dir/logs/'+i, self.work_dir+'/middle_dir/normal_train_best_model_dir'))
    ...
    def predict(self, X, **kwargs):
        ...
        for i in range(len(X)):
            ...
            image_info_list = self.frcnn.detect_image(image)
    ...
    def persist_model(self, fs, step_path):
        ...
        explanation = [
            ...
            # ChartData('netron', 'netron', None, {"path": "data/"+self.best_model_pth})
        ]
    ...
    def prepare(self, step_path, **kwargs):
        ...
        self.frcnn = FRCNN(self.step_path)
