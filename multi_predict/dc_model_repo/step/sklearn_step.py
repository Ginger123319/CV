...
class SKLearnDCEstimator(BaseEstimator):
    ...
    def transform_data(self, X, calc_max_proba=True, calc_all_proba=False, **kwargs):
        ...
        if self.learning_type == LearningType.BinaryClassify:
            binary_threshold = kwargs.get("binary_threshold", None)
...
class SKLearnDCTuningEstimator(BaseEstimator):
    ...
    def transform_data(self, X, calc_max_proba=True, calc_all_proba=False, **kwargs):
        ...
        proba = None
        if calc_max_proba:  # 只要满足任何一个条件就计算
            if self.learning_type in [LearningType.Clustering,LearningType.Regression]:
                logger.info("Ignore calc_max_proba for this learning_type: {}".format(self.learning_type))
