...
class Estimator(UserDefinedEstimator):
    def __init__(self, model):
        ...
        UserDefinedEstimator.__init__(self=self,
                                      ...
                                      algorithm_name="Classifier_multi auto annotation")
        self.model = model
...
def select_hard_example(df_pred, strategy_name, query_num):
    ...
    if strategy_name == "EntropySampling":
        ...
        # 此处将负号去掉了，就是取现在值小的就是熵大的
        log_probs_pos = torch.log(probs)
        log_probs_neg = torch.log(1 - probs)
        uncertainties = (probs * log_probs_pos + (1 - probs) * log_probs_neg).sum(1)
        hard_sample_list = uncertainties.sort()[1][:query_num].tolist()
    elif strategy_name == "MarginSampling":

        # 暂定为一张图片的每个二分类预测差值的绝对值之和
        uncertainties = abs(2 * probs - 1).sum(1)
        hard_sample_list = uncertainties.sort()[1][:query_num].tolist()
