...
class TransNetV2:
    ...
    def predict_frames(self, frames: np.ndarray):
        ...
        def chunked(iterator, batch_size):
            """Yield 'n' sized chunks from an iterable"""
            iterator = iter(iterator)  # 确保传入的是一个迭代器
            while True:
                chunk = list(islice(iterator, batch_size))
                if not chunk:
                    break
                yield np.concatenate(chunk, axis=0)
        
        # 使用chunked函数进行切分
        batch_size = 16
        input_iterator = chunked(input_iterator(), batch_size)

        # modify source code
        for i, inp in enumerate(input_iterator):
            single_frame_pred, all_frames_pred = self.predict_raw(inp)
            predictions.append((single_frame_pred.numpy()[:, 25:75, 0],
                                all_frames_pred.numpy()[:, 25:75, 0]))

            print("\r[TransNetV2] Processing video frames {}/{}".format(
                min((i+1) * batch_size * 50, len(frames)), len(frames)
            ), end="")
        print("")

        single_frame_pred = np.concatenate([single_.flatten() for single_, all_ in predictions])
        all_frames_pred = np.concatenate([all_.flatten() for single_, all_ in predictions])
