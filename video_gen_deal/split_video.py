...
def convert2frameTimeCode(cut_points, frame_rate):
    ...
    return timecodes, timecodes_serial
...
def split2scenes(video_path, output_dir):
    ...
    if scene_list:
        result_dict = {}
        scene_list, timecode_serial = convert2frameTimeCode(scene_list, fps)
...
if __name__ == '__main__':
    ...
    # 常用的视频格式后缀
    video_extensions = ['.mp4', '.avi', '.mov', '.mkv', '.flv', '.wmv']
    video_files = (str(f) for f in Path(directory).rglob('*') if f.is_file() and f.suffix in video_extensions)
