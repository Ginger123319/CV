...
class Detector(torch.nn.Module):  # 定义侦测模块
    ...
    def __init__(self):
        ...
        self.net.load_state_dict(torch.load(cfg.param_path, map_location=device))  # 加载网络参数
        self.net.eval()  # 固化参数即使得BatchNormal不在测试的时候起作用

    def forward(self, input, thresh, anchors):  # 定义前向运算，并给三个参数分别是输入数据，置信度阀值，及建议框
    ...
    def _filter(self, output, thresh):  # 赛选过滤置信度函数，将置信度合格留下来
    ...
    def _parse(self, idxs, vecs, t, anchors):  # 定义解析函数，并给4个参数分别是上面筛选合格的框的索引，9个值（中心点偏移和框的偏移即类别数），
        ...
        x2 = x1 + w  # 计算框右下角x的坐标
        y2 = y1 + h  # 计算框右下角y的坐标
        out = torch.stack([confidence, x1, y1, x2, y2, _classify], dim=1)  # 将置信度坐标和类别按照一轴即列的方向重组堆叠在一起
        return out
...
if __name__ == '__main__':
    ...
    for image_file in img_name_list:
        ...
        out_value = detector(img, 0.3, cfg.ANCHORS_GROUP)  # 调用侦测函数并将数据，置信度阀值和建议框传入
        ...
        for j in range(4):  # 循环判断类别数
            ...
            boxes.append(tool1.nms(_boxes))  # 对同一类别做nms删掉不合格的框，并将框添加进列表
        ...
        for box in boxes:  # 3种特征图的框循环框torch.cat([boxes_13, boxes_26, boxes_52], dim=0),循环3次
            ...
            try:
                ...
                c, x1, y1, x2, y2, cls = box[0, :]  # 将置信度和坐标及类别分别解包出来
                # print(c,x1, y1, x2, y2)
                # print(int(cls.item()))
