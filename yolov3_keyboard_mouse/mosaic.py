            ...
            if i == 3:
                ...
                if y1 <= cuty <= y2:
                    ...
                    if y2 - y1 < 5:
                        ...
                        # 更新坐标信息
            bboxes[i][0][index] = [cls, x1, y1, x2, y2]  # 更新第i张图的第index个检测框的坐标
...
def get_random_data(image_list, input_shape):
    ...
    for frame_list in image_list:
        ...
        cx = (box[0, :, 1] + box[0, :, 3]) // 2  # 检测框中心点的x坐标
        cy = (box[0, :, 2] + box[0, :, 4]) // 2  # 检测框中心点的y坐标
        ...
        # 调整检测框的宽高
        bw = (box[0, :, 3] - box[0, :, 1]) * nw / iw  # 修改后的检测框的宽高
        bh = (box[0, :, 4] - box[0, :, 2]) * nh / ih
        ...
        if index == 1:  # 右上图像
            box[0, :, 1] = cx - bw // 2 + w - nw  # x1
            box[0, :, 2] = cy - bh // 2  # y1
            box[0, :, 3] = cx + bw // 2 + w - nw  # x2
            box[0, :, 4] = cy + bh // 2  # y2

        if index == 2:  # 左下图像
            box[0, :, 1] = cx - bw // 2  # x1
            box[0, :, 2] = cy - bh // 2 + h - nh  # y1
            box[0, :, 3] = cx + bw // 2  # x2
            box[0, :, 4] = cy + bh // 2 + h - nh  # y2

        if index == 3:  # 右下图像
            box[0, :, 3] = cx - bw // 2 + w - nw  # x1
            box[0, :, 4] = cy - bh // 2 + h - nh  # y1
            box[0, :, 1] = cx + bw // 2 + w - nw  # x2
            box[0, :, 2] = cy + bh // 2 + h - nh  # y2
    ...
    imageio.imwrite(r'F:\Detect morse\data_test\img\{0}.jpg'.format(j), modify_image_copy)
    # 绘制修正后的检测框
    tag = []
    # file = open(r'../data_test/label/{0}.txt'.format(j), 'w')
    # file.write(str(str(j) + '.jpg'))
    for boxes in new_boxes:
        ...
        for box in boxes:
            ...
            tag.append(list(box))
            file.write((' ' + str(cls) + " " + str(x1) + ' ' + str(y1) + " " + str(x2) + " " + str(y2)))
    # print(str(str(j) + '.jpg' + ' ' + str(cls) + " " + str(x1) + ' ' + str(y1) + " " + str(x2) + " " + str(y2)))
    # file = open(r'../data_test/label/{0}.txt'.format(j), 'w')
    # file.write(str(str(j) + '.jpg' + str(tag)))
...
if __name__ == '__main__':
    ...
    for j in range(1, 601):
        ...
        for i in range(1, 5):
            ...
            # 某张图片位置及其对应的检测框信息
            image_path = os.path.join(image_dir, k + ".jpg")
            annotation_path = os.path.join(annotation_dir, k + ".xml")
            ...
            for obj in root.findall('object'):
                ...
                # 保存每张图片的检测框信息
                image_box.append([cls, left, top, right, bottom])  # [[x1,y1,x2,y2],[..],[..]]box[0, :, 3]

            # 保存图像及其对应的检测框信息
            # image_box.append(cls)
            image_list.append([image, image_box])

            image_save = image_list[0]
